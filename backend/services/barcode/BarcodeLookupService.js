...
class BarcodeLookupService {
  ...
  async ensureCacheSchema() {
    ...
    try {
      await this.pool.query(ddlPrimary);
    } catch (erroCreateCache) {
      const msg = String(erroCreateCache?.message || '').toLowerCase();
      const syntaxIssue = String(erroCreateCache?.code || '') === '42601' || msg.includes('syntax error');

      if (!syntaxIssue) {
        throw erroCreateCache;
      }

      await this.pool.query(ddlFallback);
    }
